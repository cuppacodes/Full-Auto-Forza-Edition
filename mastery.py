...
def run(cfg: dict, stop_event: threading.Event,
    ...
    log_cb(f'  Using template set: {res} -> {folder}')
    ...
    while not stop():
        ...
        if nav_keys:
            log_cb(f"  Navigating: {' '.join(k.upper() for k in nav_keys)}...")
            for key in nav_keys:
                pydirectinput.press(key)
                time.sleep(0.4)
            time.sleep(0.3)
        else:
            log_cb("  Loop 1 — starting at row1 col1.")

        # ── Open action menu ──────────────────────────────────
        log_cb("  Pressing Enter to open action menu...")
        ...
        if not matched:
            log_cb("  Action menu not detected — skipping.")
            continue
